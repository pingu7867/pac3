local tracked_proxy
do -- performance data - bar chart divisions
	local AVERAGE_WINDOW_SIZE = 200
	local frame_data = {}
	...
	local function average(tbl, str)
		local sum = 0
		for i=1,AVERAGE_WINDOW_SIZE,1 do
			if tbl[i] then
				if tbl[i][str] then
					sum = sum + tbl[i][str]
				end
	...
	local divisions_colors = {
		pre_setup = HSVToColor(0,0.7,1),
		run_expressions = HSVToColor(200,0.7,1),
		process_results = HSVToColor(100,0.7,1),
		apply_to_parts = HSVToColor(50,0.7,1),
	}

	local function DrawBarChart(x,y, w,h, data)
		local name_translates = {"pre_setup", "run_expressions", "process_results", "apply_to_parts"}

		if not data then return end
		local data2 = {}
		local i_indexed_data = {}
		local sum = 0

		for name,value in pairs(data) do sum = sum + value end
		for name,value in pairs(data) do
			data2[name] = value / sum
			if name == "pre_setup" then
				i_indexed_data[1] = value / sum
			elseif name == "run_expressions" then
				i_indexed_data[2] = value / sum
			elseif name == "process_results" then
				i_indexed_data[3] = value / sum
			elseif name == "apply_to_parts" then
				i_indexed_data[4] = value / sum
			end
		end

		for i,value in ipairs(i_indexed_data) do
			--print(name_translates[i], "is " .. 100*value .. "% of the total")
			local slice_w = w * value
			local slice_x = x
			surface.SetDrawColor(divisions_colors[name_translates[i]])
			surface.DrawRect(math.floor(slice_x),y,math.ceil(slice_w),h)
			x = x + slice_w
		end
	end

	pac.RemoveHook("HUDPaint", "proxy_tracker")
	local perf_division_index = 0
	pac.AddHook("HUDPaint", "proxy_tracker", function()
		if not pace.request_proxy_stats then return end
		if pace.request_proxy_stats ~= "stats" then return end
		if pace.current_part.ClassName ~= "proxy" then return end

		pace.performance_tracked_part = pace.current_part
		...
		local checkpoints = tracked_proxy.checkpoints
		tracked_proxy.checkpoints = tracked_proxy.checkpoints or {}
		if not checkpoints then return end

		pace.FlashNotification("frames: " .. #frame_data, 0.2)
		if #frame_data > AVERAGE_WINDOW_SIZE then table.remove(frame_data,1) end
		local this_frame_data = {}
		...
		surface.DrawText(tracked_proxy:GetName() .. " thinktime data: ")
		local sum = 0
		for i=1,#checkpoints,1 do
			local tbl = checkpoints[i]
			if not tbl then continue end
			if not tbl.delta then continue end
			this_frame_data[tbl.name] = tbl.delta
			y = y + 12
			sum = sum + tbl.delta
			surface.SetTextPos(x, y)
			surface.SetTextColor(divisions_colors[tbl.name] or Color(255,255,255))
			surface.DrawText(tbl.name .. " : " .. format(average(frame_data, tbl.name) * 1000000) .. " microseconds")
		end
		surface.SetTextColor(white)
		this_frame_data.total = sum
		table.insert(frame_data, this_frame_data)
		y = y + 18
		surface.SetTextPos(x, y)
		surface.DrawText("total : " .. format(sum * 1000000) .. " microseconds")
		y = y + 18
		surface.SetTextPos(x, y)
		surface.DrawText("averaged total : " .. format(average(frame_data, "total") * 1000000) .. " microseconds")
		DrawBarChart(x,y + 20, 400, 30, {
			pre_setup = average(frame_data, "pre_setup"),
			run_expressions = average(frame_data, "run_expressions"),
			process_results = average(frame_data, "process_results"),
			apply_to_parts = average(frame_data, "apply_to_parts")
		})
		perf_division_index = 0
	end)
end
local ease_aliases = {}
...
do -- grapher - line chart
	...
	local function PlanOutSamplePoints(override_range)
	...
	local function SampleData(part, variable_x, runtime_data)
		...
		next_data_sample = CurTime() + runtime_data.refresh

		part.predict_samplehold = {}
		part.predict_samplehold_prev = {}
		local stime = SysTime()
	...
	pac.AddHook("HUDPaint", "proxy_draw_graph", function()
		...
		if IsValid(tracked_proxy.pace_tree_node) then
			...
			local runtime_data = {
				...
				refresh = pace.proxygraph_properties.panels["refresh"]:GetValue(),
				step = steps,
				--ppp = 1,--points per pixel
				scroll = false,
				randomness = pace.proxygraph_properties.panels["randomness"]:GetValue(),
				input_bounds = input_bounds,
				max_samples = pace.proxygraph_properties.panels["max_samples"]:GetValue()
			}
	...
	function pace.OpenProxyGrapher(part)
		...
		pnl:SetSize(500,500)
		pnl:SetPos(ScrW()-500,ScrH()-500)
		x_btn = vgui.Create("DButton", pnl)
		x_btn:SetSize(480/3,20) x_btn:SetPos(10 + (0 * 480/3),30)
		x_btn:SetText("x")
		y_btn = vgui.Create("DButton", pnl)
		y_btn:SetSize(480/3,20) y_btn:SetPos(10 + (1 * 480/3),30)
		y_btn:SetText("y")
		z_btn = vgui.Create("DButton", pnl)
		z_btn:SetSize(480/3,20) z_btn:SetPos(10 + (2 * 480/3),30)
		z_btn:SetText("z")
		...
		properties_pnl:SetSize(480,440) properties_pnl:SetPos(10,50)
		...
		local variable_name_pnl = pace.CreatePanel("properties_string", main_panel)
			properties_pnl:AddKeyValue("GraphVariable", variable_name_pnl)
		...
		local samples_slider = pace.CreatePanel("properties_number")
			properties_pnl:AddKeyValue("MaxSamples",samples_slider)
			samples_slider:SetValue(5000)
			samples_slider:PostInit()
		local step_slider = pace.CreatePanel("properties_number")
			properties_pnl:AddKeyValue("StepPerSample",step_slider)
			step_slider:SetValue(0.1)
			step_slider:PostInit()
		local randomness_slider = pace.CreatePanel("properties_number")
			properties_pnl:AddKeyValue("SamplingRandomness",randomness_slider)
			randomness_slider:SetValue(0.1)
			randomness_slider:PostInit()
		local refresh_slider = pace.CreatePanel("properties_number")
			properties_pnl:AddKeyValue("RefreshDelay",refresh_slider)
			refresh_slider:SetValue(0.2)
			refresh_slider:PostInit()
		local asymptote_slider = pace.CreatePanel("properties_number")
			properties_pnl:AddKeyValue("AsymptoteThreshold",asymptote_slider)
			asymptote_slider:SetValue(100)
			asymptote_slider:PostInit()

		if true then
			samples_slider:SetValue(saved_vgui_values["max_samples"])
			refresh_slider:SetValue(saved_vgui_values["refresh"])
			...
			randomness_slider:SetValue(saved_vgui_values["randomness"])

			saved_vgui_values = {
				max_samples = samples_slider:GetValue(),
				refresh = refresh_slider:GetValue(),
				...
				randomness = randomness_slider:GetValue()
			}
		end
		...
		pnl2:SetSize(240,80) pnl2:SetPos(0,20)
