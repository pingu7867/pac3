...
local print_updatetimes = CreateClientConVar("pac_tree_log_refreshes","1", true, false)
		...
		if code == MOUSE_RIGHT then
			...
			if node.part.fake_root then
				...
				menu2:AddOption(L"move up to a fake root node", function()
					node.part.fake_root = true
					pace.fake_root_nodes = pace.fake_root_nodes or {}
					pace.fake_root_nodes[node.part] = node.part
					node.part.no_populate = false
					node.part.no_populate_because_of_fakenode = true
			...
			menu2:AddOption(L"focus on this part", function()
				local parts = {}
				local children = {}
				for i,v in pairs(node.part:GetChildrenList()) do
					children[v] = v
				end
				for i,v in pairs(pac.GetLocalParts()) do
					v.no_populate = false
	...
	for key, part in pairs(tbl) do
		...
		if not part:HasParent() or children then
			...
			if part.dormant_node then
				...
				part_node.Icon:SetImage("icon16/collision_off.png")
				part:SetSmallIcon(part.Icon)
			end
